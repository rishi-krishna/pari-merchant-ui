...
type AuthStep = 'logged-out' | 'password-ok' | 'verified';
...
export class AuthFlowService {
  ...
  readonly canAccessMpin = computed(() => this.state().step === 'password-ok' || this.state().step === 'verified');
  ...
  async verifyMpin(mpin: string): Promise<boolean> {
    if (!/^\d{6}$/.test(mpin) || !this.state().stepUpToken) {
      return false;
    }
